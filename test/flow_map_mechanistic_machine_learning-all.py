...
# Load the model
#model_path = os.path.join(current_directory, joblib_ml_files[1])  

# Load the model using joblib.load()
#ml_model = joblib.load(model_path)

#if not isinstance(ml_model, xgb.XGBClassifier):
    #raise ValueError("Model is not an XGBClassifier")

#if hasattr(ml_model, 'booster_'):
...
best_configuration_list     = [(0.71,0.42,1.5), (0.72,0.42,1.5), (0.25, 0.47, 0.83),(0.25, 0.47, 0.83), (1.54, 0.33,0.5), (0.71, 0.25, 0.5)]



# Create a subdirectory called "FlowMap" if it doesn't exist
if not os.path.exists("FlowMap-All"):
    os.makedirs("FlowMap-All")

# Iterate through each file
for p, read_file in enumerate(file_names):
# Iterate through ML Models 
    for q, model_trained in enumerate(trained_models_names):
        # load ML Model
        ml_model      = joblib.load(os.path.join(current_directory, joblib_ml_files[q]))
        #print(trained_models_names[q])
        # Create the plot name by combining the filename without the .csv extension and the trained model name
        
        if best:
            plot_name = f"{os.path.splitext(read_file)[0]} - {model_trained} - best"
        else: 
            plot_name = f"{os.path.splitext(read_file)[0]} - {model_trained} - standard"
        ...
        for p in range(len(T_machine_learning_filtered)):
            # Separate x and y coordinates from the points list
            if T_machine_learning_filtered[p]: # sublist is not empty
        ...
        plt.xlabel(r' Superficial Gas velocity $U_{SG}$  $[\frac{m}{s}]$', size='14')
        plt.ylabel(r' Superficial Liquid velocity $U_{SL}$  $[\frac{m}{s}]$', size='14')
